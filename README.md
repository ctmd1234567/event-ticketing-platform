...
[![Tests](https://img.shields.io/badge/verification-147%20tests%20passed-2EA44F)](#验证证据)
...
默认产品的交易核心仍为同步 MySQL 事务。支付成功和订单关闭在同一事务写入站内通知 Outbox；RabbitMQ 只承载通知副作用，不创建核心订单。第 9 项补齐停机恢复、有限消费重试、DLQ 与管理员审计重驱；旧 Voucher 实验已退出当前应用，源码与复现材料可从 Git 历史追溯。
...
2026-09-19 使用 Microsoft OpenJDK 21.0.7 和 Maven 3.9.16 验证 V1 候选（历史结果）：
...
当前 V2 整合版在 2026-09-25 使用 Java 21.0.7、Maven 3.9.16 完成 `clean test` 和 `clean -Pinfrastructure verify`：64 个默认测试、83 个 Testcontainers 集成测试，合计 147 个，均为 0 失败、0 错误、0 跳过；新生成的 Surefire/Failsafe XML 报告通过必需套件门禁。完整证据链见 [第 12 项验收记录](docs/verification/CHECKLIST-12-V2-ACCEPTANCE.md)。历史 V1 命令、Demo、数据库审计与限制见 [V1 验收记录](docs/verification/V1-VERIFICATION.md) 和 [Event 下单基线结果](docs/verification/results/event-order-creation-baseline-20260919.md)。Flyway 11.7.2 对 MySQL 8.4 仍有版本认证提示；迁移与断言实际通过，这不是生产兼容性认证。
...
├── service/       最小身份服务
└── config/        Security、MyBatis 与 Event 通知配置

src/main/resources/db/migration/   Flyway V1～V6 不可回写；V7～V10 为前向迁移
...
- 当前完成：V1 Core Trading 与 V2 第 8～12 项；证据分别见下列验收记录
...
- V2 第 12 项：旧实验运行代码退出、数据保留迁移与工程验收；见 [第 12 项记录](docs/verification/CHECKLIST-12-V2-ACCEPTANCE.md) 和 [旧实验归档](docs/history/LEGACY-VOUCHER-ARCHIVE.md)
...
历史高并发工程实验（旧版 Voucher/Outbox/RabbitMQ 写链路，包含失败冷启动轮次、限制和原始证据）：[1500 RPS 实验记录](docs/verification/CONCURRENCY-EXPERIMENT-1500-RPS-2026-09-18.md)。该实验不代表 Event 性能、生产 SLA 或长期稳定性。
