...
当前已实现活动、场次、票档的创建/发布/查询、带服务端价格快照和库存预占的同步订单创建与本人查询，以及未支付订单取消、数据库超时关单、幂等库存释放和重启补扫。支付网关、退款、支付未知态及晚到支付补偿尚未实现；架构文档中的相应接口仍是后续目标。
...
- **自动化验证**：当前工作区已通过 38 项默认测试；2 项既有隔离集成测试覆盖 Flyway、真实 MySQL、Redis、RabbitMQ 与 1000 请求库存竞争，2 项真实 MySQL 生命周期测试覆盖 30 秒超时、取消/关单及下单/关单竞争、库存守恒和重启补扫。
...
应用启动时由 Flyway 按 `V1` 至 `V5` 顺序迁移全新空库。已有本地库仅在确认已经包含历史基础表和订单/Outbox 升级后，才按版本 `2` 建立 baseline 并继续执行 `V3`、`V4`、`V5`；Flyway clean 已禁用。测试种子只位于 `src/test/resources`，不会写入开发数据库。
