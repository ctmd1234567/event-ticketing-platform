...
[本地 Maven 报告](docs/engineering/results/local-maven-reports-20260925.txt)记录 65 个默认测试和 83 个 Testcontainers 集成测试，共 **148 个，0 失败、0 错误、0 跳过**。覆盖真实 MySQL 与 RabbitMQ、确定性并发和支付/关单竞态、Broker 停机、Redis 故障及进程恢复。合并后的本地 `mvn -Pinfrastructure verify` 退出码 0；[报告摘要](docs/engineering/results/local-maven-reports-20260925.txt)保留套件明细。[测试范围与限制](docs/engineering/TESTING.md)。
