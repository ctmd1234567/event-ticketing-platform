...
On 2026-09-25 this repository state was checked with Windows Java 21.0.7 and Maven 3.9.16: `mvn test` exited 0 with 65 Surefire tests; `mvn -Pinfrastructure verify` exited 0 with 65 Surefire and 82 Failsafe tests. Together the infrastructure run reports **147 tests, zero failures, zero errors, zero skipped**. The [local Maven report summary](results/local-maven-reports-20260925.txt) retains suite details. Hosted CI is a separate run and is not claimed here.
