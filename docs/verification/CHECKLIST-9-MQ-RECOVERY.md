...
日期：2026-09-23。基线：`2阶段` 分支、`77de49a2340f39faa3505e4b362558690b33cfa5`；第 9 项首次提交为 `920b8ca88dc49206a4ffbc93afe3569874185981`。前置第 8 项见 [验收记录](CHECKLIST-8-EVENT-NOTIFICATIONS.md)。只修改 Event 通知副作用，第 10 项用户退款与对账未实施。
...
- `POST /api/v1/admin/notifications/{eventId}/redrive` 仅 ADMIN 可访问，要求非空原因。`MANUAL_REQUIRED` 发布事件原位重置为 `PENDING`；已 `PUBLISHED` 但缺失通知的事件按原 eventId 重新发往预期拓扑。重驱前拒绝无原 Outbox 行和已存在通知。发布确认后返回 `CONFIRMED`，实际效果仍需查询通知表；若上次 `CONFIRMED` 后仍无通知，管理员核对后可留下新操作记录并再次重驱。异常结果记为 `UNCERTAIN`，人工核对后才可再次操作。并发重驱用 `REDRIVING` 状态和 60 秒租约互斥；失去进程后可重新领取，重复发送由消费幂等兜底。
...
`920b8ca` 后续收尾复验：IDEA `EventNotificationIT` 14 started / 14 finished / 0 failed，退出码 0；日志 `C:\Users\ctmd12334567\AppData\Local\JetBrains\IntelliJIdea2026.1\tmp\ij_run__EventNotificationIT_5027487497414479137.log`。新用例模拟上次重驱获 Broker Confirm 但通知仍缺失，验证管理员再次重驱原 eventId、新旧操作记录均保留、最终只产生一条通知。此结果仍不证明 Broker Confirm 等于最终通知效果。
