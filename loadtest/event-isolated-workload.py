...
def request(index, tier, user_base, run_id, base_url):
    ...
    req = urllib.request.Request(base_url + "/api/v1/orders", data=body, method="POST",
                                 headers={"Authorization": "Bearer " + token,
                                          "Idempotency-Key": f"isolated-{run_id}-{index:06d}",
...
def main():
    ...
    sql("\n".join(fixture))
    lua = "for i=0,tonumber(ARGV[2])-1 do local u=tonumber(ARGV[1])+i; local k='login:token:'..string.format('%032x',u); redis.call('HSET',k,'id',string.format('%.0f',u),'nickName','event'); redis.call('EXPIRE',k,3600) end; return tonumber(ARGV[2])"
...
if __name__ == "__main__":
    ...
    MYSQL = os.environ.get("EVENT_ISOLATED_MYSQL_CONTAINER", "event-isolated-mysql")
    REDIS = os.environ.get("EVENT_ISOLATED_REDIS_CONTAINER", "event-isolated-redis")
    main()
